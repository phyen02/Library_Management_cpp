...
class thedocgia{
	...
	/*a/12>=1=>nam+b =>3+15=18/12=1;namhh=nam+b(1); thanghhh=6;
					02/03/2012
					02/06/2013;
	a/12<1;thanhh=a;namhh=nam;*/
	void tinhngayhh(){
		if(((thang+HieuLuc)/12)>1){
			ngayhh=ngay;
			if(((thang+HieuLuc)%12)==0){
				thanghh=12;
				namhh=nam+((thang+HieuLuc)/12)-1;
			}
			if(((thang+HieuLuc)%12)>0){
				thanghh=((thang+HieuLuc)%12);
				namhh=nam+((thang+HieuLuc)/12);
			}
		...
		if(((thang+HieuLuc)/12)<1){
			ngayhh=ngay;
			thanghh=(thang+HieuLuc);
			namhh=nam;
		}
		cout<<"\nNgay het han :"<<ngayhh<<"/"<<thanghh<<"/"<<namhh;
	}
	int Sanphamhethansudung(){
			...
			int Nam=namhh -namhientai;
			int Thang=thanghh-thanghientai-1;
			int Ngay=ngayhh-ngayhientai;
			if(Nam<0){
				return 1;
			} 
			if(Nam==0){
...
class QLthedocgia{
	...
	public:
        ...
        void xuatngayhh(){
			system("cls");
        	cout<<"------So The Doc Da Het Han------";
        	for(int i=0;i<TD.size();i++){
        		int m=TD.at(i)->Sanphamhethansudung();
				if(m>0){
					TD.at(i)->xuat(); 
				}
			}
...
int main(){
    ...
    cout  << setw( 42 ) << "( N ) Nhap Danh sach Sach"
		  ...
		  << setw( 48 ) << "( K ) The Doc Het Han "<< endl << endl
		  
		  << setw( 41 ) << "( Q ) Thoat chuong trinh" << endl << endl;
		  SET_COLOR(4);
